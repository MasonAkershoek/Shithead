...
function CardTable.new()
    ...
    self.deck:shuffle()
    -- Card Pile
    self.cardPile = CardPile.new(G.SCREENVARIABLES["GAMEDEMENTIONS"].x / 2,
        (G.SCREENVARIABLES["GAMEDEMENTIONS"].y / 2 - 100))
    ...
    -- PlayButton
    self.playButton = UIButton.new((G.SCREENVARIABLES["GAMEDEMENTIONS"].x * .87),
        (G.SCREENVARIABLES["GAMEDEMENTIONS"].y * .87), 200, 100,
        { color = "RED", action = "playButton", text = "Play!", radius = "10" })
...
function CardTable:gameLogic()
    ...
    elseif G.gamestate == "TURN" then
        ...
        if winData ~= nil then
            self.winner = winData[1]
            G.EVENTMANAGER:emit("makeWinBox")
            G:setState(5)
        else
