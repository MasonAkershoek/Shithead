...
function MainMenu.new()
    ...
    self.testBox = UIBox.new(
        ...
        350, 
        {
            ...
            borderSize=20,
            alignment="center",
            positions={Vector.new(G.SCREENVARIABLES["GAMEDEMENTIONS"].x/2, -200), Vector.new(G.SCREENVARIABLES["GAMEDEMENTIONS"].x/2, 230)},
            contents={UILabel.new(
                G.SCREENVARIABLES["GAMEDEMENTIONS"].x/2,
                G.SCREENVARIABLES["GAMEDEMENTIONS"].y/2,20,
                {
                    alignment="center",
                    text="mason lksdjflkdsjf lds lfls kdjfls kdjflskd jfls lkl"
...
function MainMenu:exit()
    ...
    if self.exitDone and not self.flag then self.buttonBox:setPos(G.SCREENVARIABLES["GAMEDEMENTIONS"].x/2, G.SCREENVARIABLES["GAMEDEMENTIONS"].y+500) self.flag = true end
...
function MainMenu:update(dt)
    ...
    if self.buttonBoxTimer:isExpired() then
        --self.buttonBox:setPos(nil, G.SCREENVARIABLES["GAMEDEMENTIONS"].y-100)
        --self.buttonBoxTimer:stopTimer()
    elseif self.buttonBoxTimer.stopped then
        --self.buttonBox:updatePos(self.buttonBox.contents, true)
    end
    ...
    self.testBox:update(dt)
end

function MainMenu:draw()
    love.graphics.draw(self.versionText, 100,G.SCREENVARIABLES["GAMEDEMENTIONS"].y-100)
